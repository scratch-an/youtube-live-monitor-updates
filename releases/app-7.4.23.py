...
DEFAULT_CONFIG = {
    ...
    # 同じ主題の説明は、200文字前後なら途中で分けず一投稿にまとめる。
    "post_assistant_max_chars": 260,
    ...
    "archive_speaker_ranges": {
        ...
        "TuI4euaLfuE": [
            ...
            {"start": "00:00:40", "end": "00:03:50", "speaker": "片山さつき 財務大臣"},
            {"start": "00:03:51", "end": "00:04:37", "speaker": "記者"},
...
def merge_without_adjacent_repetition(left, right):
    """Whisperの隣接断片で繰り返された文・語句を一度だけ残して結合する。"""
    left = re.sub(r"\s+", " ", str(left)).strip()
    right = re.sub(r"\s+", " ", str(right)).strip()
    # 同じ短句が同一断片内で連続したWhisper反復も一度だけ残す。
    for _ in range(3):
        deduped = re.sub(r"(.{4,40}?)(?:[、,\s]+)\1", r"\1", right)
        if deduped == right:
            break
        right = deduped
    if not left:
    ...
    if compact_right and compact_right in compact_left[-max(80, len(compact_right)):]:
        return left
    if compact_left and compact_left in compact_right:
        return right
    max_overlap = min(len(left), len(right), 120)
    for size in range(max_overlap, 7, -1):
        if left[-size:].replace(" ", "") == right[:size].replace(" ", ""):
            return normalize_text(left + right[size:])
    # 文として同一の先頭句が連続した場合も一回にする。
    repeated = re.match(r"^(.{6,60}?)(?:[、,。\s]+)\1", right)
    if repeated:
        right = right[repeated.end(1):].lstrip(" 、,。")
...
def correct_with_official_reference(text, reference):
    ...
    instructions = (
        ...
        "入力全体を一続きの発言として照合し、固有名詞、数字、金融・財政用語、同音異義語、重複、"
        "およびチャンク境界で脱落した語句を直してください。"
        "公式文と高い確率で連続一致する場合は、その一致範囲内に限り、欠けた文頭・文中を補完できます。"
        "例として、人名は会見対象者と公式文の表記を優先し、『消費減税』『歳出歳入』『当初予算と補正予算』"
        "など、音の近い不自然な認識語を公式表記へ直します。"
    ...
    try:
        ...
        # 公式文との連続一致が強い場合、Whisperが落とした文頭を戻せる余地を持たせる。
        if not 0.50 <= ratio <= 2.4:
            return original
...
def transcribe_chunks(
    ...
    def submit_post_text(text, language, speaker=None, force=False):
        ...
        if text:
            post_buffer = merge_without_adjacent_repetition(post_buffer, text)
        ...
        while True:
            ...
            if wav_ready:
                try:
                    ...
                    normalized_segments = [
                        (normalize_text(s.text, video_id), total_offset + s.start,
                         total_offset + s.end)
                        for s in segments
                        if normalize_text(s.text, video_id)
                    ]

                    # 公式資料との照合はWhisperの短い断片ごとではなく、今回認識した
                    # 40秒前後を一続きにして行う。これにより文脈、文頭の脱落、
                    # 固有名詞を公式文の連続箇所から安全に特定しやすくする。
                    if source_language == "ja" and current_reference and normalized_segments:
                        combined = ""
                        for segment_text, _, _ in normalized_segments:
                            combined = merge_without_adjacent_repetition(combined, segment_text)
                        corrected = correct_with_official_reference(combined, current_reference)
                        corrected = normalize_text(corrected, video_id)
                        block_start = normalized_segments[0][1]
                        block_end = normalized_segments[-1][2]
                        blocks = [(corrected, block_start, block_end)] if corrected else []
                    else:
                        blocks = normalized_segments

                    for raw_segment, segment_start, segment_end in blocks:
                        ...
                        for index, part in enumerate(parts):
                            if index == len(parts) - 1:
                                ...
                                piece_end = min(
                                    segment_end,
                                    cursor + (segment_end - segment_start) * fraction,
                                )
                    ...
                    for raw, start, end in recognized_pieces:
                        ...
                        if not pending_text:
                            ...
                            if candidate_speaker != pending_speaker:
                                ...
                                pending_text = merge_without_adjacent_repetition(
                                    pending_text, raw
                                )
                                pending_end = end
