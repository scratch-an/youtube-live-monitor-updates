...
def fetch_official_archive_reference(video_id, out_dir=None):
    ...
    try:
        ...
        response.raise_for_status()
        source = re.sub(r"(?is)<(?:script|style).*?>.*?</(?:script|style)>", " ", response.text)
        ...
        print(f"⚠️ 公式会見概要を取得できないため通常認識を続けます: {_translation_error_summary(e)}")
        return ""
            ...
            if wav_ready:
                try:
                    ...
                    for s in segments:
                        ...
                        if source_language == "ja" and official_reference:
                            raw_segment = correct_with_official_reference(
                                raw_segment, official_reference
                            )
