...
def select_official_reference(text, state):
    ...
    for start in range(max(0, cursor - 2), min(len(units), cursor + 20)):
...
def transcribe_with_retry(model, wav, video_id=None, source_language="ja"):
    ...
    if first and not looks_hallucinated(joined):
        return first, False

    reason = "認識結果が空" if not first else "異常反復を検出"
    print(f"🔁 {reason}。条件を変えて再認識します: {wav.name}")
    # 再試行は強い語彙誘導を外し、beamも控えめにする。
    segments2, _ = model.transcribe(
        str(wav),
        language=source_language,
        # 最初の認識が空だった場合は、VADの誤判定による欠落を避ける。
        vad_filter=bool(first),
        beam_size=5,
        temperature=0.2,
        condition_on_previous_text=False,
        initial_prompt=None,
...
def transcribe_chunks(
    ...
    if CONFIG.get("speaker_labeling", True):
        print("👥 話者ラベル: ON（タイトル判定 + 本文推定 + 軽量声質補助）")
                ...
                try:
                    ...
                    for s in segments:
                        ...
                        for index, part in enumerate(parts):
                            ...
                            recognized_pieces.append((part, cursor, piece_end, signature))
                            if reference_speaker:
                                recognized_pieces[-1] = (
                                    part, cursor, piece_end, signature, reference_speaker
                                )
                            cursor = piece_end
